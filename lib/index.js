...
const createBatches = (currentPriorityLevel) => {
  ...
  if (newWatchers.length > 0) {
    ...
    if (getWatchersAtLevelSize(ESMongoSync.options.watchedCollections, priorityLevel + 1) === 0) {
    ...
    if (!ESMongoSync.Oplog.stream) {
      ...
      ESMongoSync.Oplog.on('update', Meteor.bindEnvironment(doc => {
        ...
        if (watcher) {
          let op = doc.o;
          if(_.has('$set')){
            // when operation contains $set operation
            update(watcher, doc.o2._id, op.$set);
          } else if (_.has('$unset')){
            // when operation contains $unset
            throw new Meteor('elasticsync-op-not-handled', 'ESMongoSync: update operation $unset not yet handled');
          } else {
            // when operation is passed a whole document
            update(watcher, doc.o2._id, op);
          }
        }
      }));
